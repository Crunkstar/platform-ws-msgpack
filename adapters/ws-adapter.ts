...
export class WsAdapter extends AbstractWsAdapter {
  ...
  public create(
    port: number,
    options?: any & { eventKey?: string, namespace?: string; server?: any },
  ): any {
    const { server, eventKey, ...wsOptions } = options;
  ...
  public bindMessageHandler(
    ...
    try {
      const message = decode(buffer.data);
      const messageHandler = handlers.find(
        handler => handler.message === message[this.eventKey],
      );
